// MPU9150 Library Code
// https://github.com/kriswiner/MPU-9150
// Date: 10.02.2016 (last change: 30 Jun 2014)
// Changes by Henrik Kunzelmann
...
enum InitError {
	IError_WrongID,
	IError_WrongID_Magnetometer,
	IError_Fail_SelfTest,
	IError_None
};
...
public:
	InitError init()
	{
		...
		Log::debug("MPU9150", "self test done");
		// schauen ob SelfTest bestanden ist
		for (int i = 0; i < 6; i++) {
			if (SelfTest[i] >= 1.0f)
				return IError_Fail_SelfTest;
		}

		Log::debug("MPU9150", "calibrate");
	...
	void update(float* gyroX, float* gyroY, float* gyroZ, 
				...
				float* magnetX, float* magnetY, float* magnetZ,
